...
export default function ActivityList({ activities, initialLimit }: ActivityListProps) {
  ...
  if (activities.length === 0) {
    return (
      <div className="text-center py-10 rounded-xl" style={{ background: 'var(--dash-surface)', border: '1px solid var(--dash-border)' }}>
        <div className="w-12 h-12 mx-auto rounded-full flex items-center justify-center mb-3" style={{ background: 'var(--dash-surface-2)' }}>
        ...
        <p className="text-xs" style={{ color: 'var(--dash-text-3)' }}>Aucune activité pour le moment</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between mb-1 px-1">
        <h2 className="text-base font-bold" style={{ color: 'var(--dash-text)' }}>Activité récente</h2>
        ...
        {displayedActivities.map((activity, index) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-medium" style={{ color: 'var(--dash-text)' }}>
                {activity.type === 'tip' ? 'Pourboire reçu' : 'Nouvel avis'}
              </p>
              {activity.type === 'review' && activity.comment && (
                <p className="text-[10px] truncate" style={{ color: 'var(--dash-text-2)' }}>
            ...
            <div className="text-right">
              {activity.type === 'tip' ? (
                ...
                <div className="flex items-center gap-0.5">
                  ...
                  <span className="font-semibold text-sm" style={{ color: 'var(--dash-text)' }}>{activity.rating}</span>
                </div>
              )}
            </div>
          </div>
