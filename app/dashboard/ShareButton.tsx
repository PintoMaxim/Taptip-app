  ...
  return (
    <button
      ...
      className={`flex-1 h-16 flex flex-col items-center justify-center gap-1.5 rounded-xl border active:scale-[0.98] transition-all ${
        copied ? 'bg-emerald-50 border-emerald-200 text-emerald-600' : 'bg-gray-50 border-gray-100 text-gray-600'
      }`}
    >
      {copied ? <CheckIcon /> : <LinkIcon />}
      <span className="text-xs font-medium">{copied ? 'Lien copié !' : 'Lien de bio'}</span>
    </button>
  )
}
